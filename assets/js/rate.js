...
function getCurrencyValue(base,to,value,localSite){
    ...
    if(valueArray.length > 1){
        for(i=0;i<valueArray.length;i++){
            querystring[i] = '&base='+base+'&to='+to+'&value='+((valueArray[i])/100);
            //alert(querystring[i]);
        }
    }
                        ...
                        case 'dashboard':
                           // alert('show: '+index);
                            if(index == 0){
                                ...
                                document.getElementById('goal-month').innerHTML = formatter.format(obj.newValue);
                            }
                            if(index == 1){
                                document.getElementById('goal-total').innerHTML = formatter.format(obj.newValue);
                            }
                        break;
                        default:
