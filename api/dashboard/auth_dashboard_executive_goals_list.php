...
if(array_key_exists('auth_api',$_REQUEST)){
    ...
    $goal_amount_column = "round(sum(g.goal_amount_int),2)";
    if($_REQUEST['date']!==''){
        if($filters != '')
            $filters .= " AND ";
        ...
        $monthsubfilter         = " goal_month = ".(int)$month." AND ";
        $goal_amount_column = "round((goal_amount_int),2)";
    ...
    g.goal_month,
    g.goal_amount_int,
    CASE WHEN pps.status_percent = 100 THEN 
        round(sum(
        CASE WHEN (pps.cost_".$currency_selected." = '0' OR pps.cost_".$currency_selected." IS NULL) AND pps.amount_".$currency_selected." IS NOT NULL THEN 
            (pps.amount_".$currency_selected." * 70)/100 
        ELSE 
            CASE WHEN pps.cost_".$currency_selected." IS NULL THEN 0 ELSE pps.cost_".$currency_selected." END
        END
        ),2)  
    ELSE 
        sum(0) 
    ...
    CASE WHEN pps.status_percent = 100 THEN 
        round(sum(
        CASE WHEN (pps.cost_".$currency_selected."_int = '0' OR pps.cost_".$currency_selected."_int IS NULL) AND pps.amount_".$currency_selected."_int IS NOT NULL THEN 
            (pps.amount_".$currency_selected."_int * 70)/100 
        ELSE 
            CASE WHEN pps.cost_".$currency_selected."_int IS NULL THEN 0 ELSE pps.cost_".$currency_selected."_int END
        END
        ),2) 
    ELSE 
        sum(0) 
    END as utility_cost_int,
    CASE WHEN pps.status_percent = 100 THEN 
        round(sum(
        CASE WHEN (pps.cost_per_month_".$currency_selected." = 0 OR pps.cost_per_month_".$currency_selected." IS NULL) AND pps.amount_per_month_".$currency_selected." IS NOT NULL THEN 
            (pps.amount_per_month_".$currency_selected." * 70)/100 
        ELSE 
            CASE WHEN pps.cost_per_month_".$currency_selected." IS NULL THEN 0 ELSE pps.cost_per_month_".$currency_selected." END
        END
        ),2) 
    ELSE 
        sum(0) 
    END as utility_cost_per_month,
    CASE WHEN pps.status_percent = 100 THEN 
        round(sum(
        CASE WHEN (pps.cost_per_month_".$currency_selected."_int = 0 OR pps.cost_per_month_".$currency_selected."_int IS NULL) AND pps.amount_per_month_".$currency_selected."_int IS NULL THEN 
            (pps.amount_per_month_".$currency_selected."_int * 70)/100 
        ELSE 
            CASE WHEN pps.cost_per_month_".$currency_selected."_int IS NULL THEN 0 ELSE pps.cost_per_month_".$currency_selected."_int END
        END
        ),2) 
    ELSE 
        sum(0) 
    END as utility_cost_per_month_int,
    CASE WHEN pps.status_percent < 100 THEN 
        round(sum(
        CASE WHEN (pps.cost_".$currency_selected." = '0' OR pps.cost_".$currency_selected." IS NULL) AND pps.amount_".$currency_selected." IS NOT NULL THEN 
            (pps.amount_".$currency_selected." * 70)/100 
    ...
    ELSE 
        sum(0) 
    END as pipe_cost_per_month,
    CASE WHEN pps.status_percent < 100 THEN 
        round(sum(
        CASE WHEN (pps.cost_per_month_".$currency_selected."_int = 0 OR pps.cost_per_month_".$currency_selected."_int IS NULL) AND pps.amount_per_month_".$currency_selected."_int IS NOT NULL THEN 
            (pps.amount_per_month_".$currency_selected."_int * 70)/100 
        ELSE 
            CASE WHEN pps.cost_per_month_".$currency_selected."_int IS NULL THEN 0 ELSE pps.cost_per_month_".$currency_selected."_int END
        END
    ...
    ELSE 
        sum(0) 
    END as pipe_cost_per_month_int,
    round(sum(
    CASE WHEN (pps.cost_".$currency_selected." = '0' OR pps.cost_".$currency_selected." IS NULL) AND pps.amount_".$currency_selected." IS NOT NULL THEN 
        (pps.amount_".$currency_selected." * 70)/100 
    ELSE 
    ...
    CASE WHEN pps.amount_".$currency_selected." IS NOT NULL THEN round(sum(pps.amount_".$currency_selected."),2) ELSE sum(0) END as amount,
    CASE WHEN pps.amount_".$currency_selected."_int IS NOT NULL THEN round(sum(pps.amount_".$currency_selected."_int),2) ELSE sum(0) END as amount_int,
    CASE WHEN (pps.status_percent AND pps.amount_".$currency_selected." IS NOT NULL) = 100 THEN 
        round(sum(pps.amount_".$currency_selected."),2) 
    ELSE 
        sum(0) 
    END as utility_amount,
    CASE WHEN (pps.status_percent = 100 AND pps.amount_".$currency_selected."_int IS NOT NULL) THEN 
        round(sum(pps.amount_".$currency_selected."_int),2) 
    ELSE 
        sum(0) 
    END as utility_amount_int,
    CASE WHEN (pps.status_percent < 100 AND pps.amount_".$currency_selected." IS NOT NULL) THEN 
        round(sum(pps.amount_".$currency_selected."),2) 
    ELSE 
        sum(0) 
    END as pipe_amount,
    ...
    ELSE 
        sum(0) 
    END as pipe_amount_int,
    CASE WHEN pps.amount_per_month_".$currency_selected." IS NOT NULL THEN round(sum(pps.amount_per_month_".$currency_selected."),2) ELSE sum(0) END as amount_per_month,
    CASE WHEN pps.amount_per_month_".$currency_selected."_int IS NOT NULL THEN round(sum(pps.amount_per_month_".$currency_selected."_int),2)  ELSE sum(0) END as amount_per_month_int,
    CASE WHEN (pps.status_percent = 100 AND pps.amount_per_month_".$currency_selected." IS NOT NULL) THEN 
        round(sum(pps.amount_per_month_".$currency_selected."),2) 
    ELSE 
        sum(0) 
    END as utility_amount_per_month,
    CASE WHEN (pps.status_percent = 100 AND pps.amount_per_month_".$currency_selected."_int IS NOT NULL) THEN 
        round(sum(pps.amount_per_month_".$currency_selected."_int),2) 
    ELSE 
        sum(0) 
    END as utility_amount_per_month_int,
    CASE WHEN (pps.status_percent < 100 AND pps.amount_per_month_".$currency_selected." IS NOT NULL) THEN 
        round(sum(pps.amount_per_month_".$currency_selected."),2) 
    ELSE 
        sum(0) 
    END as pipe_amount_per_month,
    CASE WHEN (pps.status_percent < 100 AND pps.amount_per_month_".$currency_selected."_int IS NOT NULL) THEN 
        round(sum(pps.amount_per_month_".$currency_selected."_int),2) 
    ELSE 
        sum(0) 
    END as pipe_amount_per_month_int,
    '".$currency_selected."' as currency,
    pps.quantity,
    pps.is_active";
    ...
    $tableOrView    = "view_proposals pps 
    LEFT JOIN
    (
    ...
    SELECT uuid,goal_month,goal_year,sum(goal_".$currency_selected.") as goal_amount_int 
    FROM view_goals 
    WHERE  ".$subfilter." 
    GROUP BY uuid,goal_month
    ) AS g
    ON g.uuid = pps.user_id ";

    $groupBy        = " GROUP BY pps.user_id,SUBSTR(CONCAT('00',g.goal_month),LENGTH(g.goal_month)+1,2)";
    ...
    if($rs){
        $return = array('response'=>'OK','data'=>$rs,'sql'=>$sql);
    } else {
        if($numberOfRows <= 0)
