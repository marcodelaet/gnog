...
<div class='form-<?=strtolower($moduleName)?>-container'>
    <div class="form-container">
        <div class="form-header">Import <?=$moduleName?>s from XLSX file</div>
        <form name='<?=strtolower($moduleName)?>' method="post" enctype="multipart/form-data" class="needs-validation" action="http://gnogcrm.local/api/providers/auth_provider_csv_upload.php" novalidate>
            <div class="inputs-form-container">
                <div class="input-group mb-3">
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" id="<?=strtolower($moduleName)?>-file" name="<?=strtolower($moduleName)?>_file" accept=".xlsx" onchange="document.getElementById('<?=strtolower($moduleName)?>-file-label').innerHTML = this.value.split(/[|\/\\]+/)[2];" aria-describedby="<?=strtolower($moduleName)?>-file">
                        <label class="custom-file-label" id="<?=strtolower($moduleName)?>-file-label" for="inputGroupFile01">Choose xlsx file</label>
                    </div>
                </div>
            </div>
            <div class="inputs-form-container">
            ...
            <div class="inputs-button-container">
                <button class="button" name="btnSave" type="button" onClick="handleSubmitXLSX(<?=strtolower($moduleName)?>)" >Upload list of <?=strtolower($moduleName)?>s</button>
            </div>
        </form>
    </div>    
</div>